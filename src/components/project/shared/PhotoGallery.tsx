      ...
      <div className={`grid ${gridCols[columns as keyof typeof gridCols]} gap-4 ${className}`}>
        {images.map(image => (
          ...
          >
            ...
            {/* Airbnb-style favorite button - always visible */}
            {onToggleFavorite && (
              <Button
                variant="ghost"
                size="icon"
                className="absolute top-2 right-2 rounded-full bg-black/30 hover:bg-black/50 text-white z-10"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <Button
              variant="ghost"
              size="icon"
              className="absolute bottom-2 right-2 rounded-full bg-black/30 hover:bg-black/50 text-white z-10 opacity-0 group-hover:opacity-100 transition-opacity"
              onClick={(e) => {
                e.stopPropagation();
